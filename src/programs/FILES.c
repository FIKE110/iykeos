...
static int input_mode = 0; /* 0=none, 1=new file, 2=new dir */
...
static int focus_index = 0;   /* 0=close btn, 1=back, 2=new file, 3=new dir, 4+=file list */
#define FOCUS_CLOSE_BTN   0
#define FOCUS_BACK_BTN    1
#define FOCUS_NEW_FILE    2
#define FOCUS_NEW_DIR     3
#define FOCUS_FILE_START  4
...
static void refresh_file_list(void) {
    file_count = os_api->get_file_list(file_list_str);
    scroll_offset = 0;
    selected_file = -1;
...
static void draw_toolbar(int x, int y, int w, int mx, int my) {
    ...
    /* New File button - keyboard focus support */
    int hover_nf = is_hover(mx, my, x + BTN_NEW_FILE_X, y, BTN_NEW_FILE_W, 1);
    int focus_nf = (keyboard_mode && focus_index == FOCUS_NEW_FILE);
    uint8_t nf_attr = (hover_nf || focus_nf) ? ATTR_TOOLBAR_HOV : ATTR_TOOLBAR;
    os_api->vgraphics_put_string(x + BTN_NEW_FILE_X, y, "+File", nf_attr);
    
    /* New Dir button - keyboard focus support */
    int hover_nd = is_hover(mx, my, x + BTN_NEW_DIR_X, y, BTN_NEW_DIR_W, 1);
    int focus_nd = (keyboard_mode && focus_index == FOCUS_NEW_DIR);
    uint8_t nd_attr = (hover_nd || focus_nd) ? ATTR_TOOLBAR_HOV : ATTR_TOOLBAR;
    os_api->vgraphics_put_string(x + BTN_NEW_DIR_X, y, "+Foldr", nd_attr);
    
    /* Separator */
    os_api->vgraphics_put_char(x + BTN_NEW_DIR_X + BTN_NEW_DIR_W + 1, y, '|', ATTR(COLOR_DGRAY, TOOLBAR_BG));
    
    /* Path display */
    char path_display[40];
    os_api->strcpy(path_display, " Path: ");
    os_api->strcat(path_display, current_path);
    os_api->vgraphics_put_string(x + BTN_NEW_DIR_X + BTN_NEW_DIR_W + 3, y, path_display, ATTR_TOOLBAR);
...
static void draw_input_modal(int mx, int my) {
    int modal_x = (SCREEN_WIDTH - MODAL_WIDTH) / 2;
    int modal_y = (SCREEN_HEIGHT - MODAL_HEIGHT) / 2;
    
    /* Shadow effect */
    // os_api->vgraphics_draw_rect_fill(modal_x + 1, modal_y + 1, MODAL_WIDTH, MODAL_HEIGHT, COLOR_BLACK);
    
    /* Window background */
    os_api->vgraphics_draw_rect_fill(modal_x, modal_y, MODAL_WIDTH, MODAL_HEIGHT, MODAL_BG);
    
    /* Title bar */
    os_api->vgraphics_draw_rect_fill(modal_x, modal_y, MODAL_WIDTH, 1, MODAL_TITLE_BG);
    os_api->vgraphics_draw_rect_fill(modal_x, modal_y + MODAL_HEIGHT - 1, MODAL_WIDTH, 1, MODAL_TITLE_BG);
    
    /* Title text */
    const char* title = "Input";
    if (input_mode == 1) title = " Create New File ";
    if (input_mode == 2) title = " Create New Folder ";
    int title_x = modal_x + (MODAL_WIDTH - os_api->strlen(title)) / 2;
    os_api->vgraphics_put_string(title_x, modal_y, title, ATTR(COLOR_WHITE, MODAL_TITLE_BG));
    
    /* Prompt text */
    const char* prompt = input_mode == 1 ? "Enter filename:" : "Enter folder name:";
    os_api->vgraphics_put_string(modal_x + 2, modal_y + 2, prompt, ATTR_MODAL_TEXT);
    
    /* Input box with border */
    os_api->vgraphics_draw_rect_fill(modal_x + 2, modal_y + 4, MODAL_WIDTH - 4, 1, INPUT_BG);
    os_api->vgraphics_put_string(modal_x + 2, modal_y + 4, input_buffer, ATTR_INPUT);
    
    /* Cursor indicator */
    if (input_len < MAX_INPUT_LEN) {
        os_api->vgraphics_put_char(modal_x + 2 + input_len, modal_y + 4, '_', ATTR(COLOR_DGRAY, INPUT_BG));
    }
    
    /* Help text with key indicators */
    os_api->vgraphics_put_string(modal_x + 2, modal_y + MODAL_HEIGHT - 2, 
                                  "[Enter] OK    [Esc] Cancel", ATTR(COLOR_LGRAY, MODAL_BG));
}
...
static action_t handle_toolbar_click(int x, int y, int w, int mx, int my) {
    ...
    if (is_hover(mx, my, x + BTN_NEW_FILE_X, y, BTN_NEW_FILE_W, 1)) {
        return ACTION_NEW_FILE;
    }
    if (is_hover(mx, my, x + BTN_NEW_DIR_X, y, BTN_NEW_DIR_W, 1)) {
...
static void handle_input_mode(char c) {
    if (c == '\n') {
    ...
    } else if (c >= 32 && c <= 126 && input_len < MAX_INPUT_LEN) {
        /* Regular character */
        ...
        case KEY_ENTER:
            ...
            if (keyboard_mode) {
                if (focus_index == FOCUS_CLOSE_BTN) {
                    ...
                    refresh_file_list();
                    focus_index = FOCUS_FILE_START;
                } else if (focus_index == FOCUS_NEW_FILE) {
                    input_mode = 1;
                    clear_input();
                } else if (focus_index == FOCUS_NEW_DIR) {
                    input_mode = 2;
                    clear_input();
                } else {
                    /* File selected */
...
static void app_loop(void) {
    ...
    while (1) {
        ...
        if (input_mode) {
            draw_input_modal(mouse_x, mouse_y);
        }
